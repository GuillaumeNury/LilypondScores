...
async function compileFile(file, format, verbose, compileThemes) {
  
  const allThemes = getThemes();
  const themesToCompile = compileThemes ? allThemes : [allThemes[1]];

  for (const theme of themesToCompile) {
    await compileSingleFile(file, format, verbose, theme);
  }
}

function compileSingleFile(file, format, verbose, theme) {
  const dist = DIST_PATH + '/' + theme.folder;
