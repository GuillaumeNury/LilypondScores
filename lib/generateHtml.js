...
const [draftScores, archivedScores, finalScores] = scores
  // Get matching scores
  .reduce((acc, score) => {
  ...
  .reduce(
    ([drafts, archived, finals], score) =>
      score.draft
        ? [[...drafts, score], archived, finals]
        : score.archived
        ? [drafts, [...archived, score], finals]
        : [drafts, archived, [...finals, score]],
    [[], [], []]
  );
...
for (const theme of themes) {
    writeFileSync(join(DIST_FOLDER, `${theme.folder}.html`), renderAll(theme));
}
writeFileSync(INDEX_HTML, renderAll(themes.find(t => t.name === 'Normal')));
