...
const [draftScores, archivedScores, percusScores, snareScores] = partitionArray(
    ...
    // Get matching scores
    .reduce((acc, score) => {
        const matchingFile = files.find(f => f.endsWith(PDF_EXT) && f.slice(0, -PDF_EXT.length) === basename(score.file).slice(0, -ILY_EXT.length));
        const midiFile = files.find(f =>
            (f.endsWith(MIDI_EXT) && f.slice(0, -MIDI_EXT.length) === basename(score.file).slice(0, -ILY_EXT.length))
            || (f.endsWith(MID_EXT) && f.slice(0, -MID_EXT.length) === basename(score.file).slice(0, -ILY_EXT.length))
        );
...
const renderScores = (scoresToRender, theme, additionnalPath) =>
                  ...
                  .map(
                    score =>
                      `<tr>
                            ...
                            <td>
                                ${score.midiFile
                                    ? `<a href="./${theme.folder + (additionnalPath || "")}/${score.midiFile}" target="_blank" title="Télécharger le fichier MIDI"><i class="fas fa-music"></i></a>`
                                    : ''}
                                <a href="./${theme.folder + (additionnalPath || "")}/${score.matchingFile}" target="_blank" title="Visualiser le fichier PDF"><i class="fas fa-eye"></i></a>
                                <a href="./${theme.folder + (additionnalPath || "")}/${score.matchingFile}" download title="Télécharger le fichier PDF"><i class="fas fa-download"></i></a>
                            </td>
                        </tr>`
                  )
                  .join("")}
...
<div class="small-12">
    ...
    ${renderScores(percusScores, theme, "/percus")}
</div>
${draftScores.length ?
    `<div class="small-12">
