...
console.log('/**/*.mid', glob.sync(`${DIST_FOLDER}/**/*.mid`));
console.log('/**', glob.sync(`${DIST_FOLDER}/**`));
...
const [draftScores, archivedScores, percusScores, snareScores] = partitionArray(
    ...
    .reduce((acc, score) => {
        ...
        const midiFile = files.find(f => f.endsWith(MIDI_EXT) && f.slice(0, -MIDI_EXT.length) === basename(score.file).slice(0, -ILY_EXT.length));
